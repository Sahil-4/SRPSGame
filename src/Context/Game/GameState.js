...
const GameState = (props) => {
    ...
    const [Choices, setChoices] = useState({ player: "Choose first", opponent: "Waiting..." });
    ...
    const sendChat = (message) => {
        try {
            let Data = { room_code: Player.room_code, message: message, author: Player.name };
            setMessages(Messages => [...Messages, { message: message, author: Player.name }]);
    ...
    return (
        <GameContext.Provider value={{ playWithBot, Alert, Player, Choices, Points, Result, setPlayer, connectToServer, Messages, sendChat, sendChoice, setDefault }}>
            {props.children}
